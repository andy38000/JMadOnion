# -*- coding: utf-8 -*-
"""
Weight Pruner V5 - Multi-Pose Optimization (Maya 2018 Compatible)
Copy and paste into Maya Script Editor to run
"""
...
class WeightPrunerV5:
    ...
    def record_current_pose(name=None, logger=None):
        ...
        if not sel:
            if logger:
                logger.append("Please select a mesh first")
            return None
        ...
        for dag_path, component in sel:
            ...
            if logger:
                logger.append("  {0}: {1} verts, {2} bones".format(mesh_name, num_verts, num_inf))

        WeightPrunerV5.recorded_poses.append(pose)

        if logger:
            logger.append("Recorded: {0} (Frame {1})".format(name, current_frame))
            logger.append("Total {0} poses".format(len(WeightPrunerV5.recorded_poses)))
    ...
    def clear_poses(logger=None):
        ...
        if logger:
            logger.append("All poses cleared")
    ...
    def remove_pose(index, logger=None):
        if 0 <= index < len(WeightPrunerV5.recorded_poses):
            ...
            if logger:
                logger.append("Removed: {0}".format(removed.name))
    ...
    def prune(dag_path, component, target_bones, threshold, max_error=0.05,
        ...
        if len(WeightPrunerV5.recorded_poses) == 0:
            if logger:
                logger.append("Error: Please record at least 1 pose first!")
                logger.append("Recommend 3-5 extreme poses")
            return 0
        
        skin_name = WeightPrunerV5.get_skin_cluster(dag_path)
        if not skin_name:
            if logger:
                logger.append("skinCluster not found")
            return 0
        
        skin_fn = WeightPrunerV5.get_skin_fn(dag_path)
        if not skin_fn:
            if logger:
                logger.append("Cannot get skinCluster")
            return 0
        ...
        if logger:
            logger.append("Mesh: {0}".format(mesh_name))
            logger.append("Bones: {0}, Verts: {1}".format(num_inf, len(elements)))
            logger.append("Target: {0} bones, Max Error: {1}".format(target_bones, max_error))
        ...
        if not poses_matrices:
            if logger:
                logger.append("Error: No pose data for {0}".format(mesh_name))
            return 0
        
        if logger:
            logger.append("Using {0} poses for optimization".format(len(poses_matrices)))
        ...
        for idx, vtx in enumerate(elements):
            if idx % progress_step == 0 and logger:
                logger.append("Processing... {0}/{1}".format(idx, len(elements)))
        ...
        if logger:
            logger.append("Applying weights...")
        ...
        if logger:
            logger.append("=" * 40)
            logger.append("Done!")
            logger.append("  Optimized: {0}".format(stats['optimized']))
            logger.append("  Skipped: {0}".format(stats['skipped']))
            logger.append("  Kept original: {0} (error exceeded)".format(stats['failed']))
            if stats["optimized"] > 0:
                avg = stats["total_error"] / stats["optimized"]
                logger.append("  Avg Error: {0:.6f}".format(avg))
...
class WeightPrunerUIV5(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        self.setStyleSheet("""
            QDialog { background: #2B2B2B; color: #E0E0E0; font-family: Arial, sans-serif; }
    ...
    def _build_ui(self):
        ...
        info = QtWidgets.QLabel(
            "Workflow:\n"
            "1. Select mesh\n"
            "2. Pose to extreme positions, click [Record Pose] for each\n"
            "3. Click [Optimize Prune] - auto find best bone combination\n\n"
            "Method: Try different bone combos, pick the one with minimum error"
        )
        ...
        btn_record = QtWidgets.QPushButton("Record Pose")
        ...
        note = QtWidgets.QLabel(
            "* Target Bones: Max influences per vertex\n"
            "* Weight Thresh: Values below this are treated as 0\n"
            "* Max Error: Keep original weights if error exceeds this"
        )
        ...
        btn_prune = QtWidgets.QPushButton("Optimize Prune (Multi-Pose)")
    ...
    def _run_prune(self):
        ...
        logger = PrintLogger(self.log)
        if not sel:
            logger.append("Please select mesh or vertices")
            return

        if len(WeightPrunerV5.recorded_poses) == 0:
            logger.append("Please record at least 1 pose first!")
            logger.append("Recommend 3-5 extreme poses")
            return
        ...
        try:
            self.log.clear()
            logger.append("=== Multi-Pose Optimize Prune V5 ===")
            ...
            logger.append("Error: {0}".format(e))
            import traceback
            traceback.print_exc()
        finally:
    ...
    def _run_analyze(self):
        ...
        PrintLogger(self.log).append("Over-limit vertices: {0}".format(len(data)))
